...
export function RestaurantEditor({
  ...
  const restaurantName = config.restaurantName || "Gourmet Delight";
  const tagline = config.tagline || "Where Every Meal is a Masterpiece";
  const restaurantLogo = config.restaurantLogo || "";
  ...
  const tickerRightMessage =
    config.tickerRightMessage || "Taste the Difference";
  ...
  const handleUpdateMenuItem = (idx: number, field: string, value: any) => {
  ...
  const handleRemoveMenuItem = (idx: number) => {
    const updated = menuItems.filter((_: any, i: number) => i !== idx);
    onConfigChange({ ...config, menuItems: updated });
  };
  ...
  return (
    <div className="space-y-8">
      {/* Restaurant Branding */}
      <CollapsibleSection title="🍽️ Restaurant Branding">
        <div className="space-y-3">
          ...
          <div>
            ...
            <ImageUploader
              images={restaurantLogo ? [restaurantLogo] : []}
              onChange={(imgs) =>
                handleFieldChange("restaurantLogo", imgs[0] || "")
              }
              maxImages={1}
        ...
        <div className="space-y-3">
          <div className="p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
            <p className="text-sm text-blue-400">
              ...
              ratio) for your restaurant gallery. Images will display on the
              right side and rotate automatically with your menu items.
            </p>
          </div>
        ...
        <div className="space-y-3">
          <div>
            ...
            <Input
              ...
              placeholder="🍽️ Fresh Ingredients • Authentic Flavors • Exceptional Service"
              className="bg-slate-700 border-slate-600 text-slate-50"
            />
          </div>
          <div>
      ...
      {/* Menu Items Section */}
      <CollapsibleSection title="🍴 Menu Items">
        <div className="space-y-3">
          ...
          {menuItems.map((item: MenuItem, idx: number) => (
            <div
              ...
              className={`bg-slate-700/50 p-4 rounded-lg space-y-3 border-2 ${
                item.isSpecial ? "border-amber-500/50" : "border-transparent"
              }`}
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-slate-400">
                  Menu Item #{idx + 1}
                  {item.isSpecial && (
                    <span className="ml-2 text-amber-400">⭐ Special</span>
                  )}
                </span>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Input
                    value={item.name}
                    onChange={(e) =>
                      handleUpdateMenuItem(idx, "name", e.target.value)
                    }
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleUpdateMenuItem(idx, "description", e.target.value)
                  }
                  placeholder="Fresh Atlantic salmon with herbs and lemon butter"
                  className="bg-slate-700 border-slate-600 text-slate-50 text-sm"
                />
              </div>
              ...
              <div>
                ...
                <select
                  value={item.category}
                  onChange={(e) =>
                    handleUpdateMenuItem(idx, "category", e.target.value)
                  }
              ...
              <div>
                ...
                <ImageUploader
                  ...
                  onChange={(imgs) =>
                    handleUpdateMenuItem(idx, "image", imgs[0] || "")
                  }
                  maxImages={1}
                  userId={currentUserId}
                  displayId={displayId}
              ...
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      handleUpdateMenuItem(idx, "available", e.target.checked)
                    }
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      handleUpdateMenuItem(idx, "isSpecial", e.target.checked)
                    }
                    className="w-4 h-4 rounded border-slate-600 bg-slate-700"
                  />
          ...
          {menuItems.length === 0 && (
            <div className="text-center py-8 text-slate-500 text-sm">
              No menu items added yet. Click "Add Menu Item" to start building
              your menu.
            </div>
          )}
          ...
          {menuItems.length > 0 && (
            <div className="p-3 bg-green-500/10 border border-green-500/30 rounded-lg mt-4">
              <p className="text-xs text-green-400">
                <strong>{menuItems.length}</strong> menu items added •{" "}
                <strong>
                  {menuItems.filter((i: MenuItem) => i.isSpecial).length}
                </strong>{" "}
                specials •{" "}
                <strong>
                  {menuItems.filter((i: MenuItem) => i.available).length}
                </strong>{" "}
                available
              </p>
            </div>
